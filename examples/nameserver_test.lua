...
local function realTest(code, h, body)
    local h, b, err = p:request(code, "localhost:9876", h, body)
    if err then
        print('----err----', err)
    else
...
realTest(REQUEST_CODE.DELETE_KV_CONFIG, { namespace = "", key = "12" })
realTest(REQUEST_CODE.GET_KV_CONFIG, { namespace = "", key = "12" })
realTest(REQUEST_CODE.QUERY_DATA_VERSION, { brokerAddr = "localhost:1234" },
...
realTest(REQUEST_CODE.GET_ROUTEINFO_BY_TOPIC, { topic = "TopicTest", })
realTest(REQUEST_CODE.GET_BROKER_CLUSTER_INFO, { topic = "TopicTest", })
...
realTest(REQUEST_CODE.GET_ALL_TOPIC_LIST_FROM_NAMESERVER, {})
realTest(REQUEST_CODE.DELETE_TOPIC_IN_NAMESRV, { topic = "TopicTest" })
realTest(REQUEST_CODE.GET_ALL_TOPIC_LIST_FROM_NAMESERVER, {})
realTest(REQUEST_CODE.GET_KVLIST_BY_NAMESPACE, { namespace = core.ORDER_TOPIC_CONFIG })
realTest(REQUEST_CODE.GET_TOPICS_BY_CLUSTER, { cluster = "DefaultCluster" })
